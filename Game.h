...
public:
    ...
    static value evaluate(std::vector<value> arr, value size) {
    ...
    std::tuple<std::vector<value>,bool> update(std::vector<value> arr, std::bitset<12> bitSet, value size, value i, value p) {
        bool changed = false;
        ...
        if (line_coord.x == 0) {
            ...
            if (x + 1 < board.getWidth() && y+1 < board.getHeight() && bitSet[board.getCoordIndex()[key1]] == 1 && bitSet[board.getCoordIndex()[key2]] == 1 && bitSet[board.getCoordIndex()[key3]] == 1) {
                changed = true;
            ...
            if (x-1 >= 0 && bitSet[board.getCoordIndex()[key1]] == 1 && bitSet[board.getCoordIndex()[key2]] == 1 && bitSet[board.getCoordIndex()[key3]] == 1) {
                changed = true;
            ...
            std::string key1 = "1 " + std::to_string(y + 1) + " " + std::to_string(x),
                ...
                key3 = "0 " + std::to_string(x + 1) + " " + std::to_string(y);
            if (x + 1<board.getWidth() && bitSet[board.getCoordIndex()[key1]] == 1 && bitSet[board.getCoordIndex()[key2]] == 1 && bitSet[board.getCoordIndex()[key3]] == 1) {
                changed = true;
                arr[x * board.getHeight() + y] = p;
            }
            ...
            if (y-1 >= 0 && bitSet[board.getCoordIndex()[key1]] == 1 && bitSet[board.getCoordIndex()[key2]] == 1 && bitSet[board.getCoordIndex()[key3]] == 1) {
                changed = true;
                arr[x * board.getHeight() + y] = p;
            }
        }
        ...
        std::tuple<std::vector<value>, bool> ans = std::tuple(arr, changed);
        return ans;
    }
    value makeBestMove() {
        ...
        recursion_count = 0;
        const value width = board.getWidth(), height = board.getHeight();

        std::vector<value> arr;
        for (value i = 0; i < board.getWidth(); i++) {
            for (value j = 0; j < board.getHeight(); j++) {
          
                arr.push_back(board.getBoardSquares()[i][j].getPlayer());
            }
        }
    ...
    value minimax(std::bitset<12> bitSet, std::vector<value> arr, value depth, bool isMaximizing, value alpha, value beta, bool show = false) {
